...
function MemUI:CreateFont(name, size, font)
...
function MemUI:DrawCircle( x, y, r, color )
	...
	local roundedBox = {}
	for i = 0, 360 do
		local posx = r * math.cos(math.rad(i)) + x
		local posy = r * math.sin(math.rad(i)) + y
		table.insert(roundedBox, { x = posx, y = posy })
	end
	surface.DrawPoly(roundedBox)
end
